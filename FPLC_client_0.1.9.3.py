...
'''
>>>>>>>>>>FPLC_client_0.1.9.3 (FPLC_controller) 
Functions:
...
class GPIOControl:
    ...
    def setup_gpio(self):
        for line_num, config in self.gpio_configs.items():
            initial_value = Value.ACTIVE if config.get('initial_state') == 'HIGH' else Value.INACTIVE
            line_settings = gpiod.LineSettings(
                direction=config['direction'],
                output_value=Value.INACTIVE if config['direction'] == Direction.OUTPUT else Value.INACTIVE,
...
if __name__ == '__main__':
    ...
    gpio_configs = {
        ...
        6: {'direction': Direction.OUTPUT, 'initial_state': 'HIGH'}, # Frac Collector "Run"
