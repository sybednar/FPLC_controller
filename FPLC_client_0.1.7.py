...
'''
>>>>>>>>>>FPLC_client_0.1.7 (FPLC_controller) 
Functions:
...
class DataAcquisition:
    ...
    def data_acquisition(self):
        ...
        def pause(self):
            print("Pausing data acquisition thread")
            self.pause_event.clear()

        def resume(self):
            print("Resuming data acquisition thread")
            self.pause_event.set()
    ...
    try:
        while True:
            ...
            if signal == "START_ADC":
                ...
                data_acquisition.pause_event.set() # Ensure the pause event is set (resume)
                if not data_acquisition.acquisition_thread.is_alive():
                    data_acquisition.start
            elif signal == "STOP_ADC":
                print("Received STOP_ADC signal")
                data_acquisition.stop_event.set() # Set the event to stop the thread
                data_acquisition.pause_event.set() # Ensure the pause event is set to allow the thread to exit
                data_acquisition.acquisition_thread.join()
                break
            elif signal == "PAUSE_ADC":
                print("Received PAUSE_ADC signal")
                data_acquisition.pause_event.clear() # Clear the event to pause the thread
            elif signal == "RESUME_ADC":
                print("Received RESUME_ADC signal")
                data_acquisition.pause_event.set() # Set the event to resume the thread
    except socket.error as e:
