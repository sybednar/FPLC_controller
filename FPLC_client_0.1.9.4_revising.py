...
'''
>>>>>>>>>>FPLC_client_0.1.9.4 (FPLC_controller) 
Functions:
...
class FractionCollector:
    def __init__(self, gpio_control):
        ...
        self.operable_in_pin = 5
        self.start_stop_pin = 6
        self.pause_resume_pin = 13
        self.operable_out_pin = 16

    def start(self):
        #self.gpio_control.set_value(self.operable_in_pin, GPIOControl.LOW)
        #time.sleep(0.05)
        self.gpio_control.set_value(self.start_stop_pin, GPIOControl.LOW)
        operable_out_status = self.gpio_control.monitor_input(self.operable_out_pin)
        print(f"GPIO16 status: {operable_out_status}")
        

    def stop(self):
        operable_out_status = self.gpio_control.monitor_input(self.operable_out_pin)
        if operable_out_status == "LOW":
            self.gpio_control.set_value(self.start_stop_pin, GPIOControl.HIGH)
            #self.gpio_control.set_value(self.operable_in_pin, GPIOControl.HIGH)
    ...
    def pause(self):
        operable_out_status = self.gpio_control.monitor_input(self.operable_out_pin)
        start_stop_status = self.gpio_control.monitor_input(self.start_stop_pin)
        print(f"Pause check - GPIO16: {operable_out_status}, GPIO6: {start_stop_status}")
        if operable_out_status == "LOW" and start_stop_status == "LOW":
            self.gpio_control.set_value(self.pause_resume_pin, GPIOControl.LOW)
            print("Fraction collector paused")

    def resume(self):
        operable_out_status = self.gpio_control.monitor_input(self.operable_out_pin)
        start_stop_status = self.gpio_control.monitor_input(self.start_stop_pin)
        pause_resume_status = self.gpio_control.monitor_input(self.pause_resume_pin)
        if operable_out_status == "LOW" and start_stop_status == "LOW" and pause_resume_status == "LOW":
            self.gpio_control.set_value(self.pause_resume_pin, GPIOControl.HIGH)
            print("Fraction collector resumed")    
    #def is_operable_in(self):


    #def is_started(self):


    #def check_operable_out(self):
