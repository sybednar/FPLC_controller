...
'''
>>>>>>>>>>FPLC_client_1.5 (FPLC_controller) 
Functions:
...
class PumpControl:
    def __init__(self, gpio_chip, gpio_line):
        #gpio_chip_path = f"/dev/{gpio_chip}"
        #if not os.path.exists(gpio_chip_path):
            #raise FileNotFoundError(f"GPIO chip device file {gpio_chip_path} does not exist.")
        
        
        self.chip = gpiod.Chip(gpio_chip)
        self.line = self.chip.get_line(gpio_line)
        self.line.request(consumer="pump_control", type=gpiod.LINE_REQ_DIR_OUT)
        self.stop_event = threading.Event()
...
class ValveControl:
    def __init__(self, gpio_chip, gpio_line):
        self.chip = gpiod.Chip(gpio_chip)
        self.line = self.chip.get_line(gpio_line)
        self.line.request(consumer="valve_control", type=gpiod.LINE_REQ_DIR_OUT)
        self.stop_event = threading.Event()
...
if __name__ == '__main__':
    ...
    pump_control = PumpControl('gpiochip0', 17)
