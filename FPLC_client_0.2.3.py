...
'''
>>>>>>>>>>FPLC_client_0.2.3 (FPLC_controller)
...
8) add mcp23017 chip handling, toggle GPA0
9) monitor server and client connection
'''
...
class MCP23017:
    def __init__(self, bus, address):
        ...
        # Set GPA0 to LOW initially
        current_value = self.bus.read_byte_data(self.address, self.OLATA)
        new_value = current_value & ~(1 << 0) # Clear bit 0 to set GPA0 to LOW
    ...
    def perform_task(self):
        ...
        while not self.stop_event.is_set():
            ...
            eluate_volume = elapsed_time * (self.flowrate/60)
                        ...
                        # Check gpio12 status and set the flag
            ...
            # Check if the fraction collector is not running after 1 second
            if elapsed_time >= 1 and not self.frac_collector_running:
                print("Fraction collector not running")
                '''Need to connect to server error signaling that fraction
                    collector is not running dialog window
                '''
...
if __name__ == '__main__':
    ...
    gpio_configs = {
        5: {'direction': Direction.OUTPUT, 'initial_state': 'HIGH'}, # Enable FPLC device "Operable(in)"
        ...
        17: {'direction': Direction.OUTPUT, 'initial_state': 'LOW'},# TEST LED TOGGLE
    ...
    try:
        while True:
            ...
            if signal.startswith("FLOWRATE:"):
                ...
                mcp.toggle_pin('A', 0)# Toggle GPA0 on MCP23017
